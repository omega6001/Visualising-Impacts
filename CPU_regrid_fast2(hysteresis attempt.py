...
def process_with_interpolated_marching_cubes_cpu(
    hdf5_folder, out_folder, bounds_min, bounds_max, globalmin_en, globalmax_en,global_thresh=0.001 ,hysteresis_margin=0.0002, 
    ...
    for frame_idx in range(len(files) - 1):
        ...
        with h5py.File(os.path.join(hdf5_folder, fname_a), 'r') as fa, \
            ...
            prev_active_mask = np.zeros(coarse_grid_size, dtype=bool)  # Reset per frame
            ...
            for s in range(interp_steps-1):
                ...
                halo_temperature_kelvin = interp_ie * 0.8e9
                print(max(halo_temperature_kelvin), min(halo_temperature_kelvin), np.mean(halo_temperature_kelvin), "halo")
                ...
                high_thresh = global_thresh + hysteresis_margin
                low_thresh = global_thresh - hysteresis_margin

                above_high = coarse_dens_grid > high_thresh
                below_low = coarse_dens_grid < low_thresh

                new_active_mask = np.where(
                    above_high, True,
                    np.where(below_low, False, prev_active_mask)
                )
                
                prev_active_mask = new_active_mask.copy()
                active_cells = np.argwhere(new_active_mask)
                print(len(active_cells),"active cells")
                def process_cell(cell):
                    ...
                    local_max_density = dens_grid.max()

                    # Check against hysteresis-aware marching cubes threshold
                    if local_max_density < (global_thresh - hysteresis_margin):
                     # Too weak: keep halo, skip mesh
                        return None, None, None, halo_pos, None, halo_colors_out, None

                    elif local_max_density < global_thresh:
                        # Borderline case: treat same for now, optionally track prev_active
                        return None, None, None, halo_pos, None, halo_colors_out, None

                    # Proceed with marching cubes as usual
                    ...
                    smoothed_mesh = laplacian_smooth(mesh, iterations=4, lam=0.4)
                    ...
                    specular = np.clip(dot_rv, 0.0, 1.0) ** 8
                    specular *= 0.4 * facing_light
                    ambient_color = np.full((verts.shape[0], 3), 0.15, dtype=np.float32)
                    base_color = np.full((verts.shape[0], 3), 0.35, dtype=np.float32)
                    lit_color = ambient_color + base_color * diffuse[:, None] + specular[:, None]

                    temperature_kelvin = vertex_ie * (grid_size[0]**3) * 3000
                    print(max(temperature_kelvin), min(temperature_kelvin), np.mean(temperature_kelvin), "vertex")
                ...
                if all_verts or all_local_pos:
                    combined_verts = np.vstack(all_verts) if all_verts else np.empty((0, 3), dtype=np.float32)
                    combined_faces = np.vstack(all_faces) if all_faces else np.empty((0, 3), dtype=np.uint32)
                    combined_normals = np.vstack(all_normals) if all_normals else np.empty((0, 3), dtype=np.float32)
                    combined_vertex_ie = np.concatenate(all_vertex_ie) if all_vertex_ie else np.empty((0,), dtype=np.float32)
                    combined_vertex_blackbody_color = np.vstack(all_vertex_blackbody_color) if all_vertex_blackbody_color else np.empty((0, 3), dtype=np.float32)

                    combined_local_pos = np.vstack(all_local_pos) if all_local_pos else np.empty((0, 3), dtype=np.float32)
                    combined_local_colors = np.vstack(all_local_colors) if all_local_colors else np.empty((0, 3), dtype=np.float32)
                    ...
                    np.savez(save_path,
                            ...
                            halo_positions=combined_local_pos,
                            vertex_internal_energy=combined_vertex_ie,
                            halo_blackbody_color=combined_local_colors,
                            vertex_blackbody_color=combined_vertex_blackbody_color)
...
if __name__ == "__main__":
    ...
    global_thresh = dense_max*0.02 #for 10^6 this is 1.382e-5 (or 0.001*)#dense_max*0.001
    hysteresis_margin = global_thresh * 0.01
